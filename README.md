...
当前 `MV-CH120-10GM`（序列号 `DB1488042`）的固定标定入口为：

```bash
bash scripts/calibrate_ch120_intrinsics.sh
```

它使用 12 × 9 方格、11 × 8 内角点、15 mm 方格边长，并将默认文件名前缀及
YAML/JSON 内的 `camera_name` 设置为 `CH120-10GM-1`。计算和保存期间程序会自动
暂停全分辨率取流，避免后台帧在界面线程繁忙时堆积内存；完成后自动恢复预览。
...
“12 mm”是镜头的标称焦距，并不表示软件可以改变光学景深。景深主要取决于光圈、
...
```bash
bash scripts/run_hikrobot_gui.sh
```
...
对于直连 GigE 相机，电脑有线网卡和相机必须处于同一 IPv4 子网。例如相机使用
`169.254.x.x/16` 链路本地地址时，可给专用有线连接设置一个未占用的
`169.254.x.x/16` 地址且不设置网关。本机的 `enp5s0` 已配置为
`169.254.100.100/16`，用于连接序列号 `DB1488042` 的 `MV-CH120-10GM`。
界面会显示相机 IP 和主机网口 IP，连接后自动探测并设置 GigE 最优包长。黑白型号
不提供白平衡节点，界面会自动将该控件禁用。
